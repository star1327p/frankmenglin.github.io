...
function display_solution() {
    ...
    sol.innerHTML += "On the other hand if you accept the number you draw (let's call it \\(x\\)), your opponent is left with \\(\\lfloor \\frac{n+2}{2} \\rfloor\\) chances to beat \\(x\\).<br>"
    sol.innerHTML += "You need your opponent to draw less than \\(x\\) in all the remaining draws to win. Therefore your chance of winning if you accept \\(x\\) is \\(x^{\\lfloor \\frac{n+2}{2} \\rfloor}\\)<br>"
    sol.innerHTML += "That means you should accept the number \\(x\\) if it satisfies \\( x^{\\lfloor \\frac{n+2}{2} \\rfloor} \\geq 1-P(n) \\).<br>"
    sol.innerHTML += "In other words, $$ A(n+1) = (1-P(n))^{ \\frac{1}{\\lfloor \\frac{n+2}{2} \\rfloor} } $$"
    sol.innerHTML += "To compute \\( P(n+1) \\), notice the probability you win before you draw this turn is (prob. of accept)\\(\\times\\)(prob. of winning if you accept)+(prob. of decline)\\(\\times\\)(prob. of winning if you decline) <br>"
    ...
    sol.innerHTML += "That makes the probability of winning if you accept \\(= \\int_{A(n+1)}^{1} x^{\\lfloor \\frac{n+2}{2} \\rfloor}f(x) dx = \\frac{1-A(n+1)^{\\lfloor \\frac{n+2}{2} \\rfloor+1}}{( \\lfloor \\frac{n+2}{2} \\rfloor +1)(1-A(n+1))} \\) <br>"
    sol.innerHTML += "Put everything together, $$ P(n+1) = (1-A(n+1)) \\times \\left( \\frac{1-A(n+1)^{\\lfloor \\frac{n+2}{2} \\rfloor+1}}{( \\lfloor \\frac{n+2}{2} \\rfloor +1)(1-A(n+1))}\\right)+A(n+1)\\times (1-P(n)) $$"
    sol.innerHTML += "<hr />"
